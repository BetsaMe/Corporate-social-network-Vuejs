<template>
  <div id="editProfil" class="row container-fluid mx-auto px-0">
    <div class="col-lg-8">
      <form
        class="row g-3 headerFeed shadow-sm p-3 mt-3"
        @submit.prevent="checkForm"
      >
        <div
          class="d-flex flex-column mt-3 justify-content-center align-items-center text-center"
        >
          <img
            ...
            height="150"
          />
          <h3 class="font-weight-bold">Tes informations</h3>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-12">
          <button class="btn btn-orange btn-sm my-3" type="submit">
            Enregistrer
          </button>
        </div>
...
export default {
  ...
  methods: {
    checkForm() {
      this.errors = [];
      if (!this.validFormat.test(this.firstName)) {
        this.errors.push("Votre prénom doit contenir entre 2 et 20 lettres");
      } else if (!this.validFormat.test(this.lastName)) {
